...
impl Generator for PowerShell {
    ...
    fn generate(&self, name: &str, bin: &str) -> Result<String, Self::Err> {
        let template = r#"Register-ArgumentCompleter -CommandName 'NAME' -ScriptBlock {
    ...
    $prevWord = if ($words.Count -gt 1 -and $words[-2]) {
        $words[-2]
    }
    else {
        'NAME'
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            return $completions | ForEach-Object { 
                $split = $_.Split("`t"); 
                $name = $split[0].Trim();
                $desc = if ($split.Length -gt 1) { $split[1] } else { $value }
