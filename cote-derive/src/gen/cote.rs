...
impl<'a> CoteGenerator<'a> {
    ...
    pub fn gen_new_app_define(&self, ident: &Ident) -> TokenStream {
        quote! {
            ...
            #[derive(Debug)]
            pub struct #ident<'a, Set, Inv, Ser> {
                parser: Option<&'a mut cote::CoteParser<Set, Inv, Ser>>,
            }

            impl<'a, Set, Inv, Ser> std::default::Default for #ident<'a, Set, Inv, Ser> {
                fn default() -> Self {
                    Self {
                        parser: None,
                    }
            ...
            impl<'a, Set, Inv, Ser> #ident<'a, Set, Inv, Ser> {
                pub fn set_inner_parser(&mut self, parser: &'a mut cote::CoteParser<Set, Inv, Ser>) {
                ...
                pub fn inner_parser(&self) -> &cote::CoteParser<Set, Inv, Ser> {
                ...
                pub fn inner_parser_mut(&mut self) -> &mut cote::CoteParser<Set, Inv, Ser> {
                    if self.parser.is_none() {
                        panic!("Got error here, should set parser for {} before using", stringify!(#ident))
                    }
    ...
    pub fn gen_sync_running_ctx(&self) -> TokenStream {
        ...
        if self.configs.has_cfg(CoteKind::Help) {
            ret.extend(quote! {
                if self.inner_parser().find_val::<bool>(#HELP_OPTION_NAME)? == &true {
                    let running_ctx = self.rctx_mut()?;
                    if sub_parser {
                        running_ctx.set_display_sub_help(true);
