...
impl<'a> Analyzer<'a> {
    ...
    pub fn gen_all(&self) -> syn::Result<TokenStream> {
        ...
        let where_clause = self.cote_generator.gen_into_parser_where_clause(true);
        let where_predicate =
            if let Some(where_predicate) = self.cote_generator.gen_where_predicate(true) {
                where_predicate
            } else {
                quote! {}
            };
        let where_predicate_original =
            if let Some(where_predicate) = self.cote_generator.gen_where_predicate(false) {
                where_predicate
            } else {
                quote! {}
            };
        let params = &self.cote_generator.get_generics().params;
        ...
        if params.is_empty() {
            Ok(quote! {
                ...
                impl <'zlifetime, S> cote::ExtractFromSetDerive<'zlifetime, S>
                    for #ident where S: aopt::prelude::SetValueFindExt, #where_predicate {
            ...
            Ok(quote! {
                #[doc=concat!("Automatic generated by cote-derive for [`", stringify!(#ident), "`].")]
                impl<'zlifetime, #params, P> cote::IntoParserDerive<'zlifetime, P> for #ident<#params> #where_clause
                {
                    fn update(parser: &mut aopt::prelude::Parser<'zlifetime, P>) -> Result<(), aopt::Error> {
                        #parser_update
                    }
                ...
                #[doc=concat!("Automatic generated by cote-derive for [`", stringify!(#ident), "`].")]
                impl <'zlifetime, #params, S> cote::ExtractFromSetDerive<'zlifetime, S> for #ident<#params>
                    where S: aopt::prelude::SetValueFindExt, #where_predicate {
                    fn try_extract(set: &'zlifetime mut S) -> Result<Self, aopt::Error> where Self: Sized {
                        #try_extract
                    }
                }
                ...
                #[doc=concat!("Automatic generated by cote-derive for [`", stringify!(#ident), "`].")]
                impl<#params> #ident<#params> where #where_predicate_original {
                    #parser_interface
                }
            })
        }
    ...
    pub fn gen_sub_app_type(&self, lifetime: Option<Lifetime>) -> syn::Result<TokenStream> {
        ...
        for sub_generator in self.sub_generator.iter() {
            let app_type = sub_generator.gen_app_type(lifetime.clone())?;
    ...
    pub fn gen_parser_settings(&self) -> TokenStream {
        ...
        if let Some(style_settings) = self.cote_generator.gen_style_settings() {
            ret.extend(style_settings);
        }
        for arg in self.arg_generator.iter() {
            ret.extend(arg.gen_nodelay_setting().into_iter());
        }
        ret
    }
    ...
    pub fn gen_parser_interface(&self) -> syn::Result<TokenStream> {
        ...
        Ok(quote! {
            pub fn into_app<'z>() -> Result<cote::CoteApp<'z, #policy_ty>, aopt::Error> #where_predicate_withz {
