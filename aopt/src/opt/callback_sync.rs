...
pub trait OptFn<S: Set>: Debug + Send + Sync {
...
pub trait OptFnMut<S: Set>: Debug + Send + Sync {
...
pub trait PosFn<S: Set>: Debug + Send + Sync {
    fn call(
        &mut self,
        uid: Uid,
        set: &S,
...
pub trait PosFnMut<S: Set>: Debug + Send + Sync {
    fn call(
        &mut self,
        uid: Uid,
        set: &mut S,
...
pub trait MainFn<S: Set>: Debug + Send + Sync {
    fn call(
        &mut self,
        uid: Uid,
        set: &S,
...
pub trait MainFnMut<S: Set>: Debug + Send + Sync {
    fn call(
        &mut self,
        uid: Uid,
        set: &mut S,
...
unsafe impl<S: Set> Send for Callback<S> {}
unsafe impl<S: Set> Sync for Callback<S> {}
...
where
    S: 'static + Set + Send + Sync,
    T: 'static + Fn(Uid, &S, OptValue) -> Result<Option<OptValue>> + Send + Sync;
...
where
    S: 'static + Set + Send + Sync,
    T: 'static + FnMut(Uid, &mut S, OptValue) -> Result<Option<OptValue>> + Send + Sync;
...
where
    S: 'static + Set + Send + Sync,
    T: 'static + Fn(Uid, &S, &str, u64, OptValue) -> Result<Option<OptValue>> + Send + Sync;
...
where
    S: 'static + Set + Send + Sync,
    T: 'static + FnMut(Uid, &mut S, &str, u64, OptValue) -> Result<Option<OptValue>> + Send + Sync;
...
where
    S: 'static + Set + Send + Sync,
    T: 'static + Fn(Uid, &S, &[&str], OptValue) -> Result<Option<OptValue>> + Send + Sync;
...
where
    S: 'static + Set + Send + Sync,
    T: 'static + FnMut(Uid, &mut S, &[&str], OptValue) -> Result<Option<OptValue>> + Send + Sync;
